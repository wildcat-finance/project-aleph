#!/usr/bin/env python3
"""
verify_anchors.py — Project Aleph
...
renderer — 444/444 at `wildcat-docs@6c94fb3` — not derived from a spec, so a
...
is suspected, and before shipping any change to `gitbook_id()`.
    ...
    python3 ingest/chunkers/verify_anchors.py --root wildcat-docs \
        [--site https://docs.wildcat.finance]

For every page in SUMMARY.md: fetch the rendered HTML, extract heading ids,
scan the local markdown through the production code path — scan_structure,
heading_text, gitbook_id, the same duplicate counter chunk_file uses — and
pair the two positionally.
...
def local_ids(root: pathlib.Path, rel: str) -> list[str]:
    """Anchor ids for one document, through the production code path —
    assign_anchors is the same function chunk_file calls."""
...
def live_ids(site: str, url: str) -> list[str] | None:
    ...
    req = urllib.request.Request(
        f"{site}/{url}".rstrip("/"),
        headers={"User-Agent": "aleph-verify-anchors/1.0"})
    try:
...
def main() -> int:
    ...
    ap.add_argument("--root", required=True, help="pinned wildcat-docs checkout")
    ap.add_argument("--site", default="https://docs.wildcat.finance")
    args = ap.parse_args()
